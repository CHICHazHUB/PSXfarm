...
function DiamondsStat()
	local totalDiamonds = Player.leaderstats[diamonds].Value
	for _, huges in pairs(Player.leaderstats[diamonds].Value) do
		if huges.id == "Diamonds" then
...
Statistics:Section('DiamondRN'); local gemsRNTXT = getSection("DiamondRN")
...
local startDiamondsCount = 0
function updateShards()
	startDiamondsCount = DiamondsStat()
	gemsRNTXT.Text = "have " .. startDiamondsCount .. " shards"
	while true do 
		task.wait(10)
		DiamondCount = getShards()
		gemsRNTXT.Text = "have " .. DiamondCount .. " shards"
	end
end

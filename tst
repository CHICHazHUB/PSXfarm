...
local DiamondCount = Player.leaderstats[diamonds].Value
function getGem()
	local totalDiamonds = Player.leaderstats[diamonds].Value
	for _, diamonds in pairs(Player.leaderstats[diamonds].Value) do
		if diamonds.id == "Diamonds" then
			totalDiamonds = Player.leaderstats[diamonds].Value
		end
	end
...
Statistics:Section('Diamonds'); local gemsRNTXT = getSection("Diamonds")
...
local formattedDiamondsCount = 0
function updateDiamonds()
	while true do
		task.wait(15)
		DiamondCount = Player.leaderstats[diamonds].Value 
		formattedDiamondsCount = formatNumber(DiamondCount)
		gemsRNTXT.Text = "have " .. formattedDiamondsCount .. " gems"
	end
end
